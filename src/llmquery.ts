...
// Define a type for Slack messages
...
export type LLMMessageProcessingResult = {
  product: "Web" | "Mobile" | "Extension";
  confidence: number;
    ...
    const visit = (nodeId: string) => {
      ...
      if (node.dependencies) {
        for (const depId of node.dependencies) {
          if (!this.nodes[depId]) {
            throw new Error(
              `Dependency "${depId}" not found for node "${nodeId}"`
            );
          }
          visit(depId);
        }
...
export const createProcessImagesNode = (
  id: string,
  files: string[]
): ProcessImagesNode => {
  return {
...
export const createQueryLLMNode = (
  ...
  text: string,
  dependencies: string[] = [],
  sayFn: (message: SlackMessage) => Promise<void>
): QueryLLMNode => {
  return {
    id,
    type: NodeType.QUERY_LLM,
    ...
    async execute(context: ExecutionContext): Promise<string> {
      ...
      console.log(
        `Querying LLM with prompt and ${imageContents.length} images`
      );
      ...
      const typedOutput: LLMMessageProcessingResult = JSON.parse(
      ...
      // Create a more friendly and conversational message
          ...
          },
          {
            ...
            elements: [
              ...
              },
              {
                ...
                style: "default", // Blue background (Slack default)
      ...
      // Store the original response for later nodes
      context.results.originalLlmOutput = typedOutput;

      // Return the raw output as a string for other nodes
...
export const createLinearSearchNode = (
  ...
  return {
    ...
    async execute(context: ExecutionContext): Promise<LlmResponse> {
      ...
      if (searchTool) {
        console.log(
          `Found ${McpToolName.SearchIssues} tool. Preparing call with query: ${this.query}`
        );
        ...
        try {
          ...
          console.log(
            `Calling MCP tool: ${searchTool.name} with params:`,
            parameters
          );
    ...
    async execute(context: ExecutionContext): Promise<string> {
      ...
      const formatSimplifiedResult = (result: any): string => {
        ...
        const formattedItems = limitedItems.map((item: LinearItem) => {
          ...
          const description =
            item.metadata?.context?.description?.snippet ?? "N/A";
      ...
      const prompt = await fs.promises.readFile(
        "src/prompts/rate_matching_tickets.txt",
        "utf-8"
      );
      ...
      const response = await llm.responses.create({
        ...
        input: [
          {
            role: "user",
            content: [{ type: "input_text", text: ratingPrompt }],
          },
        ],
      });
